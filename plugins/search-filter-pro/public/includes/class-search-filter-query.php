...
class Search_Filter_Query {
	...
	public function setup_archive_query( $query, $is_custom_query = false ) {
		...
		if ( ( $display_results_as == "archive" ) && ( $query->is_main_query() ) ) {
			...
			if ( isset( $post_types[0] ) ) {
				...
				} else if ( ( $post_type == "post" ) && ( is_home() ) ) { //this then works on the blog page (is_home) set in `settings -> reading -> "a static page" -> posts page
					$filter_query = true;
				} else {
	...
	public function add_permalink_filters() { //apply any regular WP_Query logic
		global $searchandfilter;
	...
	public function get_page_no_from_url( $url ) {
		...
		} else if ( isset( $url_args['product-page'] ) ) { //then its woocommerce product shortcode pagination

			$sf_page_no = (int) ( $url_args['product-page'] );
		} else if ( isset( $url_args['sf_paged'] ) ) { /* sf_paged check needs to be last, because we will always add it on anyway */
