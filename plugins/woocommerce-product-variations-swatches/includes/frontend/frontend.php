...
//		$attribute = $args['attribute'];
	...
	public function wvps_wp_enqueue_scripts() {
		...
		wp_localize_script( 'vi-wpvs-frontend-script', 'vi_wpvs_frontend_params', array(
			'prioritize_first_attribute' => $this->settings->get_params( 'prioritize_first_attribute' )
		) );
...
.vi-wpvs-variation-wrap.vi-wpvs-variation-wrap-{$id} .vi-wpvs-option-wrap.vi-wpvs-option-wrap-out-of-stock-attribute,
.vi-wpvs-variation-wrap.vi-wpvs-variation-wrap-{$id} .vi-wpvs-option-wrap.vi-wpvs-option-wrap-disable{z-index:1}";
					}
					switch ( $tooltip_type ) {
						case 'image':
							$css .= $this->add_inline_style(
		...
		if ( is_array( $name ) && count( $name ) ) {
			foreach ( $name as $key => $value ) {
				...
				if ( ! $get_value ) {
					...
					if ( $default > 0 ) {
						$get_value = $get_value * floor( floatval( $default ) ) / 100;
					}
					if ( in_array( $name[ $key ], array(
						...
						if ( $get_value >= floor( floatval( $min_size ) ) / 2 ) {
							$skip = true;
						}
					}

					$get_value = $get_value * floor( floatval( $reduce ) ) / 100;
